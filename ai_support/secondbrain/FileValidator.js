...
class FileValidator {
    ...
    DUPLICATE_PATTERN = /export\s+(function|class|type|const|interface)\s+(\w+)/g;
    RECOVERY_COOLDOWN = 60000; // 1 min between recovery attempts
    /**
     ...
     */
    shouldAttemptRecovery(filePath) {
     ...
     */
    validateDirectory(dirPath, extensions = ['.ts', '.tsx']) {
        const results = [];
        const scanDir = (dir) => {
            try {
